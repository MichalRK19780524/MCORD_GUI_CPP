...
//  connect(updateTableTimer, &QTimer::timeout, lanConnection, &LanConnection::tableUpdate);
  ...
  connect(ui->pushButtonBack, &QPushButton::clicked, this,
          &Widget::backClicked);
  connect(ui->pushButtonDisconnect, &QPushButton::clicked, this,
          &Widget::disconnectClicked);
  connect(ui->pushButtonDetectionSlabsNext, &QPushButton::clicked, this,
          &Widget::slabNumberSelection);
  connect(ui->pushButtonDetectionSlabBack, &QPushButton::clicked, this,
          &Widget::detectionSlabsBackClicked);
  connect(ui->pushButtonDetect, &QPushButton::clicked, this,
          &Widget::detectSlab);
  connect(ui->pushButtonOn, &QPushButton::clicked, this,
          &Widget::detectAndOnSlab);
  connect(ui->pushButtonBackSlabsChoice, &QPushButton::clicked, this,
          &Widget::disconnectClicked /*&Widget::backSlabsChoiceClicked*/);

  connect(lanConnection->getSocket(), &QTcpSocket::disconnected, this,
          &Widget::disconnected);
  connect(lanConnection->getSocket(), &QTcpSocket::errorOccurred, this,
          &Widget::connectionError);

  //    connect(setMasterSignalMapper, &QSignalMapper::mappedInt, this,
  //    &Widget::setMasterVoltageClicked); connect(onMasterSignalMapper,
  //    &QSignalMapper::mappedInt, this, &Widget::onMasterClicked);
  //    connect(offMasterSignalMapper, &QSignalMapper::mappedInt, this,
  //    &Widget::offMasterClicked);

  //    connect(setSlaveSignalMapper, &QSignalMapper::mappedInt, this,
  //    &Widget::setSlaveVoltageClicked); connect(onSlaveSignalMapper,
  //    &QSignalMapper::mappedInt, this, &Widget::onSlaveClicked);
  //    connect(offSlaveSignalMapper, &QSignalMapper::mappedInt, this,
  //    &Widget::offSlaveClicked);
  connect(this, &Widget::connectLan, lanConnection, &LanConnection::connect);
  connect(lanConnection, &LanConnection::connectionSucceeded, this,
          &Widget::showSlabsAfterLanConnection);
  connect(this, &Widget::closeLanConnection, lanConnection,
          &LanConnection::closeConnection);
  connect(lanConnection, &LanConnection::writingError, this,
          &Widget::writingErrorLanHandler);

  connect(this, &Widget::slabRequired, lanConnection, &LanConnection::getSlab);
  connect(lanConnection, &LanConnection::slabReadingCompleted, this,
          &Widget::appendSlabToModel);
  connect(lanConnection, &LanConnection::slabDataRetrieved, this, &Widget::updateSlabInModel);
  connect(model, &DetectorTableModel::slabAppended, this,
          &Widget::addWidgetsToTable);

  connect(this, &Widget::initializationRequired, lanConnection,
          &LanConnection::initAndOnSlab);
  connect(lanConnection, &LanConnection::initFailed, this,
          &Widget::initFailedLanHandler);
  connect(lanConnection, &LanConnection::onFailed, this,
          &Widget::onFailedLanHandler);
  connect(lanConnection, &LanConnection::initializationSucceeded, this,
          &Widget::refreshSlab);
  connect(this, &Widget::slabUpdateRequired, lanConnection,
          &LanConnection::updateSlab);
}
  ...
  if (state == State::DISCONNECTED) {
    ...
    if (ui->radioButtonLan->isChecked()) {
      ...
      std::transform(portList.cbegin(), portList.cend(),
                     ...
                     [](QSerialPortInfo spi) { return spi.portName(); });
...
void Widget::addSetWidgets() {
  ...
  QPushButton *setVoltageButtonMaster =
      new QPushButton("Set", setVoltageWidgetMaster);
  ...
  QPushButton *setVoltageButtonSlave =
  ...
  connect(setVoltageButtonSlave, &QPushButton::clicked, setSlaveSignalMapper,
          static_cast<void (QSignalMapper::*)()>(&QSignalMapper::map));
...
void Widget::setMasterVoltageClicked(int slabId) {
  ...
  //    QWidget * setWidget =
  //    ui->slabsTableView->indexWidget(model->findIndexOfMasterSlabSetButton(slabId));
  //    QString setVoltageText = setWidget->findChildren<QLineEdit
  //    *>().at(0)->text();
...
void Widget::setSlaveVoltageClicked(int slabId) {
  //    Slab* slab = model->findSlab(slabId);
  //    QWidget * setWidget =
  //    ui->slabsTableView->indexWidget(model->findIndexOfSlaveSlabSetButton(slabId));
  //    QString setVoltageText = setWidget->findChildren<QLineEdit
  //    *>().at(0)->text();
...
void Widget::initFailedLanHandler(quint16 id, const QString& message){
  QMessageBox::critical(this, "Init slab error", message);
}

void Widget::onFailedLanHandler(quint16 id, const QString& message){
  QMessageBox::critical(this, "On slab error", message);
}
