...
void LanConnection::closeConnection()
{
    ...
    if(socket->isOpen())
    {
        if(socket->write(QJsonDocument(CLOSE).toJson(QJsonDocument::Compact)) < 0)
        {
...
QString LanConnection::getSlab(quint16 slabId, AfeType afeType)
{
    QString result;
    Slab slab(slabId);
...
Sipm *LanConnection::getSipmTemperatureFromHub(Sipm *sipm, QJsonArray command, quint16 avgNumber)
{
    QList<double> temperatureList;
    temperatureList.reserve(avgNumber);
    for(int i = 0; i < avgNumber; ++i)
